...
with open(file_path, "r", encoding="utf-8") as f:
    for line in f:
        ...
        if match:
            ...
            left_vol_str   = match.group(4)  # "1373"
            left_ratio_str = match.group(5)  # "0.02"
            right_vol_str  = match.group(6)  # "0"
            right_ratio_str= match.group(7)  # "0.00"
            hz_str         = match.group(8)  # "10201"
...
data = list(zip(cv_vals, left_vols, right_vols))
...
plt.xlabel("CV (raw)")
plt.ylabel("Ratio")
plt.title("Left/Right Ratios vs. CV")
plt.grid(True)
plt.legend()
plt.show()
